...
struct Feature
{
   ...
   */
    ...
    std::map<StateIDType, Eigen::Vector2d, std::less<StateIDType>, Eigen::aligned_allocator<std::pair<const StateIDType, Eigen::Vector2d>>>observations;
...
bool Feature::initializePosition(const CamStateServer &cam_states)
{
    ...
    // cout << "id: " << id
    //      << " cam_poses size: " << cam_poses.size()
    //      << " observations size: " << observations.size() << endl;
