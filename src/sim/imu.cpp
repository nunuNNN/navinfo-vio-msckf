...
MotionData IMU::MotionModel(double t)
{
    ...
    Eigen::Vector3d position( 30, ellipse_y * sin( K * t) + 5,  5); // z * sin( K1 * K * t ) + 5);
    Eigen::Vector3d dp(0,  K * ellipse_y * cos(K*t), 0);// z*K1*K * cos(K1 * K * t));              // position导数　in world frame
    double K2 = K*K;
    Eigen::Vector3d ddp( 0,  -K2 * ellipse_y * sin(K*t), 0); //-z*K1*K1*K2 * sin(K1 * K * t));     // position二阶导数

    // Rotation
    double k_roll = 0.1;
    double k_pitch = 0.2;
    Eigen::Vector3d eulerAngles(k_roll*cos(t), k_pitch*sin(t), 0);   // roll ~ [-0.2, 0.2], pitch ~ [-0.3, 0.3], yaw ~ [0,2pi]
    Eigen::Vector3d eulerAnglesRates(-k_roll*sin(t), k_pitch*cos(t), 0);      // euler angles 的导数
