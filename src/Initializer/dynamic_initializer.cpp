...
bool DynamicInitializer::tryDynInit(const std::vector<StrImuData>& curr_from_last_imu,
                                FeatureMeasurePtr measure) 
{
    // 
    for(const auto& imu_msg : curr_from_last_imu)
    {
        processIMU(imu_msg);
    }

    processImage(measure);
...
void DynamicInitializer::processIMU(const StrImuData& imu_msg) 
{
    Vector3d angular_velocity, linear_acceleration;
    linear_acceleration = imu_msg.acc;
    angular_velocity = imu_msg.gyr;
    ...
    if (!first_imu) 
    {
        ...
        curr_time = imu_msg.timestamp;
    }

    double dt = imu_msg.timestamp - curr_time;
    ...
    if (frame_count != 0)
    {
        ...
        Rs[j] *= msckf_vio::getSmallAngleQuaternion(un_gyr * dt).toRotationMatrix();         
...
void DynamicInitializer::processImage(FeatureMeasurePtr measure) 
{
    if (f_manager.addFeatureCheckParallax(frame_count, measure))
        marginalization_flag = MARGIN_OLD;
    else
        marginalization_flag = MARGIN_SECOND_NEW;

    Times[frame_count] = measure->stamp;

    ImageFrame imageframe(measure);
    imageframe.pre_integration = tmp_pre_integration;
    all_image_frame.insert(make_pair(measure->stamp, imageframe));
    ...
    {
        bool result = false;
        if((measure->stamp-initial_timestamp) > 0.1) 
        {
            result = initialStructure();    
            initial_timestamp = measure->stamp;
        }
...
bool DynamicInitializer::visualInitialAlign() 
{
    ...
    {
        Matrix3d Ri = all_image_frame[Times[i]].R;
        Vector3d Pi = all_image_frame[Times[i]].T;
        Ps[i] = Pi;
        Rs[i] = Ri;
        all_image_frame[Times[i]].is_key_frame = true;
    }
    ...
    // Set rotation
    Matrix3d R_c02w = Quaterniond::FromTwoVectors(gravity_c0, -gravity_world).toRotationMatrix();
...
void DynamicInitializer::assignInitialState(msckf_vio::IMUState& imu_state) 
{
    if (!bInit) {
